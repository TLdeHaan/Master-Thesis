...
class SimModel:
    ...
    def cap_no_res(self, labda):
        monthly_blocks = 0
        x = (((labda * self.planning_horizon) ** monthly_blocks) / factorial(monthly_blocks)) * \
            exp(-(labda * self.planning_horizon))
    ...
    def cap_cons_res(self):
        daily_blocks_low, daily_blocks_high, days_low, schedule = self.cap_no_res(self.labda_sum)
    ...
    def no_res(self, seed_input):
        bl, bh, dl, s = self.cap_no_res(self.labda_sum)
                ...
                if pat == 3:
                    while i < self.planning_horizon:
                        if s[i] > 1:
                            ...
                            i += 1
            s = np.delete(s, 0)
            s = np.append(s, bh) if day % 7 == 0 or day % self.days_week > dl else np.append(s, bl)
            day += 1
        day = 1
                    ...
                    while i < self.planning_horizon:
                        if os[i] > 1:
                            ...
                            if i == self.planning_horizon:
                                at_array[pat - 1, i] += 1
            if day % self.days_week == 0 or day % self.days_week > dl:
                occ_rate[day - 1] = 1 - os[0] / obh
            else:
                occ_rate[day - 1] = 1 - os[0] / obl
            rs = np.delete(rs, 0)
            rs = np.append(rs, rb)
            os = np.delete(os, 0)
            os = np.append(os, obh) if day % self.days_week == 0 or day % self.days_week > dl else np.append(os, obl)
    ...
    def free_sel(self, seed_input):
        ...
        while day <= self.warm_up:
            ...
            for pat in a:
                ...
                if pat == 2:
                    if sum(s[:3]) > 0:
                        x = np.random.randint(1, sum(s[:3]) + 1)
                        while x > s[i]:
                            x = x - s[i]
                            i += 1
                        s[i] -= 1
                        ...
                        while i < self.planning_horizon:
                            if s[i] > 0:
                                ...
                                i += 1
                if pat == 3:
                    if sum(s[:14][s[:14] > 1]) > 1:
                        x = np.random.randint(1, sum(s[:14][s[:14] > 1]) + 1)
                        while x > s[i]:
                            if s[i] > 1:
                                x = x - s[i]
                            i += 1
    ...
    def bar(self, pol, at_runs, at, occ):
        ...
        for j in range(3):
            ...
            plt.xticks(range(0, self.planning_horizon + 3), list([0, "", "", "", "", "", "",
                                                                  7, "", "", "", "", "", "",
                                                                  14, "", "", "", "", "", "",
                                                                  21, "", "", "", "", "", "",
                                                                  28, "R"]))
...
def statistics_at(at_total, runs):
    ...
    for i in range(runs * 3):
        ...
        at_stats_runs[i, 2] = sum(at_total[i, 8:-1]) if i % 3 == 1 else np.NaN
        ...
        if i % 3 != 1:
            ...
            if at_stats_runs[i, 2] > 0:
                at_stats_runs[i, 6] = np.average(range(9, SimModel.planning_horizon + 1), weights=at_total[i, 8:-1])
                var_lim = 0
                if at_stats_runs[i, 2] > 1:
                    for j, weight in enumerate(at_total[i, 8:-1]):
                        var_lim += (weight * ((j + 9) - at_stats_runs[i, 6]) ** 2) / (at_stats_runs[i, 2] - 1)
...
setup_1 = SimModel(2.65, 0.28, 0.12)
setup_2 = SimModel(2.15, 0.78, 0.12)
setup_3 = SimModel(1.65, 1.28, 0.12)
experiments_2(setup_3, 2, 10, True, False, False, False)    # Policy 1=no_res, 2=cons_res, 3=incr_res, 4=free_sel, 5=all
experiments_2(setup_3, 3, 10, True, False, False, False)    # Policy 1=no_res, 2=cons_res, 3=incr_res, 4=free_sel, 5=all
# experiments_2(setup_3, 4, 10, True, True, False, True)    # Policy 1=no_res, 2=cons_res, 3=incr_res, 4=free_sel, 5=all